...
import { chargeCodeAPI, chargeTypeAPI } from '../services/api';
import { ChargeCode, ChargeType } from '../services/mockData';
...
function DetailField({ label, value, editable, field, required, hasError, currentItem, setCurrentItem, validationErrors, setValidationErrors, chargeTypes, language, t }: {
  label: string;
  value: string | undefined;
  editable?: boolean;
  field?: keyof ChargeCode;
  required?: boolean;
  hasError: (f: string) => boolean;
  currentItem: Partial<ChargeCode>;
  setCurrentItem: (c: Partial<ChargeCode>) => void;
      ...
      {editable && field ? (
        field === 'status' ? (
          ...
          <select
            ...
            value={currentItem.chargeTypeId || ''}
            onChange={(e) => {
              const selectedId = e.target.value;
              const selected = chargeTypes.find(ct => ct.id === selectedId);
              setCurrentItem({ ...currentItem, chargeTypeId: selectedId, chargeTypeName: selected?.name || '' });
              clearError('chargeTypeId');
            }}
          >
            <option value="">{language === 'zh' ? '请选择费用类型' : 'Select Charge Type'}</option>
            {chargeTypes.filter(ct => ct.status === 'active').map(ct => (
              <option key={ct.id} value={ct.id}>{ct.name}</option>
            ))}
          </select>
        ) : field === 'description' ? (
          <textarea
            className={`w-full px-3 py-1.5 text-sm border ${hasError(field) ? 'border-red-400 dark:border-red-500 ring-1 ring-red-100 dark:ring-red-900/30' : 'border-gray-200 dark:border-gray-700/50'} rounded-lg bg-white dark:bg-gray-900/50 dark:text-white input-themed focus:ring-2 focus:ring-offset-0 transition-shadow resize-none`}
            rows={3}
            value={(currentItem[field] as string) || ''}
            onChange={(e) => { setCurrentItem({ ...currentItem, [field]: e.target.value }); clearError(field); }}
          />
        ) : (
          <input
            type="text"
...
export default function ChargeCodePage() {
  const { t, language } = useContext(LanguageContext);
  const { themeColor } = useContext(ThemeContext);

  const [items, setItems] = useState<ChargeCode[]>([]);
  ...
  const [currentItem, setCurrentItem] = useState<Partial<ChargeCode>>({});
  const [deleteTarget, setDeleteTarget] = useState<ChargeCode | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [validationErrors, setValidationErrors] = useState<Record<string, string>>({});

  useEffect(() => { fetchItems(); fetchChargeTypes(); }, []);

  const fetchItems = async () => {
    setIsLoading(true);
    try {
      const data = await chargeCodeAPI.getChargeCodes();
      setItems(data);
    } catch (error) {
  ...
  const fetchChargeTypes = async () => {
    try {
      const data = await chargeTypeAPI.getChargeTypes();
      setChargeTypes(data);
    } catch (error) {
      console.warn('[ChargeCode] Failed to fetch charge types:', error);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await chargeCodeAPI.deleteChargeCode(deleteTarget.id);
  ...
  const handleAdd = async (item: Partial<ChargeCode>) => {
    const itemName = item.code || '';
    try {
      const created = await chargeCodeAPI.createChargeCode({
        code: item.code || '',
      ...
      setCurrentItem({});
      showToast(language === 'zh' ? '添加成功' : 'Added Successfully', language === 'zh' ? `${itemName} 已成功添加` : `${itemName} has been added`, 'add');
    } catch (error) {
      toast.error(t('addError') || '添加失败');
    }
  };

  const handleEdit = async (item: Partial<ChargeCode>) => {
    if (!currentItem.id) return;
    const itemName = currentItem.code || '';
    try {
      await chargeCodeAPI.updateChargeCode(currentItem.id, item);
      setItems(prev => prev.map(r => r.id === currentItem.id ? { ...r, ...item } as ChargeCode : r));
      setIsEditModalOpen(false);
      setCurrentItem({});
      showToast(language === 'zh' ? '更新成功' : 'Update Successful', language === 'zh' ? `${itemName} 的信息已成功更新` : `${itemName} has been updated`, 'update');
    } catch (error) {
  ...
  const filteredItems = items.filter(item =>
    ...
    item.chargeTypeName.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const dfProps = { hasError, currentItem, setCurrentItem, validationErrors, setValidationErrors, chargeTypes, language, t };
    ...
    <>
      ...
      <div className="flex items-center gap-4 pb-5 border-b border-gray-100 dark:border-gray-800/50">
        <div className="w-14 h-14 rounded-2xl avatar-container text-lg flex-shrink-0 shadow-lg">
          {getInitials(currentItem.code || '?')}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 capitalize mb-1">{t('chargeCodeLabel')}<span className="text-red-500 ml-0.5">*</span></p>
          <input
            type="text"
            className={`w-full px-3 py-1.5 text-sm font-semibold border ${hasError('code') ? 'border-red-400 dark:border-red-500 ring-1 ring-red-100 dark:ring-red-900/30' : 'border-gray-200 dark:border-gray-700/50'} rounded-lg bg-white dark:bg-gray-900/50 dark:text-white input-themed`}
            value={currentItem.code || ''}
            onChange={(e) => { setCurrentItem({ ...currentItem, code: e.target.value }); if (validationErrors.code) { const err = { ...validationErrors }; delete err.code; setValidationErrors(err); } }}
          />
        </div>
        <div className="flex-shrink-0 w-40">
          ...
          <StatusSelect
            value={(currentItem.status as string) || 'active'}
            onChange={(v) => { setCurrentItem({ ...currentItem, status: v }); }}
            t={t}
          />
        </div>
      </div>
      ...
      <div className="rounded-xl bg-gradient-to-br from-gray-50 to-gray-50/50 dark:from-gray-800/30 dark:to-gray-800/10 border border-gray-100 dark:border-gray-800/50 p-5 space-y-4">
        <div className="flex items-center gap-2 mb-1">
          <div className="w-6 h-6 rounded-md btn-gradient flex items-center justify-center">
            <i className="fa-solid fa-barcode text-[10px] text-white"></i>
          </div>
          <h4 className="text-sm font-bold text-gray-900 dark:text-white capitalize">{language === 'zh' ? '费用代码信息' : 'Charge Code Info'}</h4>
        </div>
        <DetailField {...dfProps} label={t('chargeType')} value={currentItem.chargeTypeName} editable field="chargeTypeId" required />
        <DetailField {...dfProps} label={t('chargeDescription')} value={currentItem.description} editable field="description" />
      </div>
    </>
  );
  ...
  const validateRequired = (): boolean => {
    ...
    if (!currentItem.code?.trim()) errors.code = language === 'zh' ? '费用代码' : 'Charge Code';
    if (!currentItem.chargeTypeId) errors.chargeTypeId = language === 'zh' ? '费用类型' : 'Charge Type';
    ...
    return (
      <motion.div initial={{ opacity: 0, y: -8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} transition={{ type: 'spring', duration: 0.4, bounce: 0.15 }} className="rounded-xl border border-red-200 dark:border-red-800/50 bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-950/30 dark:to-orange-950/20 p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-red-700 dark:text-red-400 mb-2">{language === 'zh' ? '请填写以下必填信息' : 'Please fill in the required fields'}</p>
            <div className="flex flex-wrap gap-1.5">
              {errorList.map((name) => (
                <span key={name} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white/80 dark:bg-gray-900/50 border border-red-200/60 dark:border-red-800/40 text-xs font-medium text-red-600 dark:text-red-400"><i className="fa-solid fa-xmark text-[9px]"></i>{name}</span>
              ))}
            </div>
          </div>
    ...
    <div className="space-y-3">
      ...
      <div className="sticky -top-4 z-10 -mx-6 px-6 pt-4 pb-3 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-white">{t('chargeCodeLabel')}</h1>
            <p className="text-sm text-gray-500">{t('manageChargeCodes')}</p>
          </div>
          <div className="mt-4 md:mt-0 flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
            <div className="relative">
              <input type="text" placeholder={t('search')} className="pl-10 pr-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700/50 bg-white dark:bg-gray-900/50 text-sm input-themed text-gray-900 dark:text-white w-full sm:w-64" value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} />
              <i className="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
            </div>
            <button className="btn-gradient rounded-xl text-sm font-medium text-white px-4 py-2 flex items-center justify-center" onClick={() => { setCurrentItem({ status: 'active' }); setValidationErrors({}); setIsAddModalOpen(true); }}>
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-gradient-to-r from-gray-50/80 to-gray-100/50 dark:from-gray-800/40 dark:to-gray-800/20 border-b border-gray-200/60 dark:border-gray-700/50">
                <th scope="col" className="px-5 py-3 text-left text-sm font-bold text-gray-900 dark:text-gray-200 capitalize">{t('chargeCodeLabel')}</th>
                <th scope="col" className="px-5 py-3 text-left text-sm font-bold text-gray-900 dark:text-gray-200 capitalize">{t('chargeDescription')}</th>
                <th scope="col" className="px-5 py-3 text-left text-sm font-bold text-gray-900 dark:text-gray-200 capitalize">{t('chargeType')}</th>
            ...
            <tbody className="divide-y divide-gray-100/80 dark:divide-gray-800/50">
              {isLoading ? (
                Array.from({ length: pageSize }).map((_, i) => (
                  <tr key={`skeleton-${i}`}>
                    <td className="px-5 py-3 whitespace-nowrap"><div className="flex items-center gap-3"><div className="skeleton w-8 h-8 rounded-lg flex-shrink-0" /><div className="skeleton h-4 w-20 rounded" /></div></td>
                    <td className="px-5 py-3 whitespace-nowrap"><div className="skeleton h-4 w-36 rounded" /></td>
                    <td className="px-5 py-3 whitespace-nowrap"><div className="skeleton h-4 w-24 rounded" /></td>
                    <td className="px-5 py-3 whitespace-nowrap"><div className="skeleton h-6 w-16 rounded-lg" /></td>
                    <td className="px-5 py-3 whitespace-nowrap text-right"><div className="flex items-center justify-end gap-2"><div className="skeleton w-8 h-8 rounded-lg" /><div className="skeleton w-8 h-8 rounded-lg" /><div className="skeleton w-8 h-8 rounded-lg" /></div></td>
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={5} className="px-5 py-12 text-center">
                    <div className="flex flex-col items-center">
                      <div className="w-12 h-12 rounded-2xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center mb-3"><i className="fa-solid fa-magnifying-glass text-gray-400 dark:text-gray-500 text-lg"></i></div>
                ...
                paginatedItems.map((item, index) => (
                  <motion.tr key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="group hover:bg-blue-50/30 dark:hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="px-5 py-3 whitespace-nowrap">
                      <div className="text-[13px] text-gray-600 dark:text-gray-400">{item.description || '-'}</div>
                    </td>
                    <td className="px-5 py-3 whitespace-nowrap">
                      <div className="text-[13px] text-gray-600 dark:text-gray-400">{item.chargeTypeName || '-'}</div>
                    </td>
                    <td className="px-5 py-3 whitespace-nowrap">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full ${item.status === 'active' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'}`}>
                    ...
                    <td className="px-5 py-3 whitespace-nowrap text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button className="w-7 h-7 rounded-lg flex items-center justify-center text-red-400 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" onClick={() => setDeleteTarget(item)} title={language === 'zh' ? '删除' : 'Delete'}><i className="fa-solid fa-trash-can text-xs"></i></button>
                      </div>
                    </td>
                  </motion.tr>
                ))
      ...
      {!isLoading && filteredItems.length > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2">
          <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
            ...
            <select className="px-2 py-1 rounded-xl border border-gray-200 dark:border-gray-700/50 bg-white dark:bg-gray-900/50 text-gray-900 dark:text-white input-themed text-sm" value={pageSize} onChange={(e) => { setPageSize(Number(e.target.value)); setCurrentPage(1); }}>
              {[5, 10, 20, 50].map(size => <option key={size} value={size}>{size}</option>)}
            </select>
            <span className="ml-2">{(safePage - 1) * pageSize + 1}-{Math.min(safePage * pageSize, filteredItems.length)} {t('of')} {filteredItems.length} {t('totalRecords')}</span>
          </div>
      ...
      {/* Detail Modal */}
      <AnimatePresence>
        {isDetailModalOpen && currentItem.id && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4" onClick={() => { setIsDetailModalOpen(false); setCurrentItem({}); }}>
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: 'spring', duration: 0.3, bounce: 0.15 }} className="modal-content relative bg-white dark:bg-gray-900/95 rounded-2xl border border-gray-100 dark:border-gray-800/80 w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6 border-b border-gray-100 dark:border-gray-800/80 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">{language === 'zh' ? '费用代码详情' : 'Charge Code Details'}</h3>
                  <span className="px-2.5 py-1 rounded-lg bg-gray-100 dark:bg-gray-800/50 text-xs font-medium text-gray-500 dark:text-gray-400">{currentItem.code}</span>
                </div>
              ...
              <div className="p-6 space-y-5">
                <div className="flex items-center gap-4 pb-5 border-b border-gray-100 dark:border-gray-800/50">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-base font-bold text-gray-900 dark:text-white truncate">{currentItem.code}</h4>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{currentItem.description || '-'}</p>
                  </div>
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-semibold rounded-lg ${currentItem.status === 'active' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400'}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${currentItem.status === 'active' ? 'bg-emerald-500' : 'bg-gray-400'}`}></span>
                    {currentItem.status === 'active' ? t('active') : t('inactive')}
                  </span>
                </div>
                <div className="rounded-xl bg-gradient-to-br from-gray-50 to-gray-50/50 dark:from-gray-800/30 dark:to-gray-800/10 border border-gray-100 dark:border-gray-800/50 p-5 space-y-4">
                  <div className="flex items-center gap-2 mb-1">
                    <div className="w-6 h-6 rounded-md btn-gradient flex items-center justify-center"><i className="fa-solid fa-barcode text-[10px] text-white"></i></div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white capitalize">{language === 'zh' ? '费用代码信息' : 'Charge Code Info'}</h4>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <DetailField {...dfProps} label={t('chargeCodeLabel')} value={currentItem.code} field="code" />
                    <DetailField {...dfProps} label={t('chargeType')} value={currentItem.chargeTypeName} field="chargeTypeId" />
                  </div>
                  <DetailField {...dfProps} label={t('chargeDescription')} value={currentItem.description} field="description" />
                </div>
              </div>
              <div className="p-6 border-t border-gray-100 dark:border-gray-800/80 flex justify-end">
                <button className="px-4 py-2 border border-gray-200 dark:border-gray-700/50 rounded-xl text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors" onClick={() => { setIsDetailModalOpen(false); setCurrentItem({}); }}>{t('close')}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
        ...
        {isAddModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4" onClick={() => setIsAddModalOpen(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: 'spring', duration: 0.3, bounce: 0.15 }} className="modal-content relative bg-white dark:bg-gray-900/95 rounded-2xl border border-gray-100 dark:border-gray-800/80 w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <div className="absolute top-0 left-0 right-0 h-1 btn-gradient" />
              <div className="p-6 border-b border-gray-100 dark:border-gray-800/80 flex items-center justify-between">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('add')} {t('chargeCodeLabel')}</h3>
                <button className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors" onClick={() => { setIsAddModalOpen(false); setCurrentItem({}); }}><i className="fa-solid fa-xmark"></i></button>
              </div>
        ...
        {isEditModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4" onClick={() => setIsEditModalOpen(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: 'spring', duration: 0.3, bounce: 0.15 }} className="modal-content relative bg-white dark:bg-gray-900/95 rounded-2xl border border-gray-100 dark:border-gray-800/80 w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <div className="absolute top-0 left-0 right-0 h-1 btn-gradient" />
              <div className="p-6 border-b border-gray-100 dark:border-gray-800/80 flex items-center justify-between">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('edit')} {t('chargeCodeLabel')}</h3>
                <button className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors" onClick={() => { setIsEditModalOpen(false); setCurrentItem({}); }}><i className="fa-solid fa-xmark"></i></button>
              </div>
              <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">{renderFormContent()}</div>
              <div className="p-6 border-t border-gray-100 dark:border-gray-800/80 flex justify-end space-x-3">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4" onClick={() => setDeleteTarget(null)}>
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: 'spring', duration: 0.3, bounce: 0.15 }} className="modal-content relative bg-white dark:bg-gray-900/95 rounded-2xl border border-gray-100 dark:border-gray-800/80 w-full max-w-sm overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <div className="p-6 text-center">
                <div className="w-12 h-12 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center mx-auto mb-4"><i className="fa-solid fa-triangle-exclamation text-red-500 text-xl"></i></div>
                ...
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mt-2">{deleteTarget.code}</p>
              </div>
              <div className="px-6 pb-6 flex gap-3">
                <button className="flex-1 px-4 py-2.5 border border-gray-200 dark:border-gray-700/50 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors" onClick={() => setDeleteTarget(null)}>{t('cancel')}</button>
                <button className="flex-1 px-4 py-2.5 bg-red-500 hover:bg-red-600 rounded-xl text-sm font-medium text-white transition-colors" onClick={handleDelete}>{t('delete')}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
