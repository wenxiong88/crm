...
import { chargeTypeAPI } from '../services/api';
import { ChargeType } from '../services/mockData';
...
function DetailField({ label, value, editable, field, required, hasError, currentItem, setCurrentItem, validationErrors, setValidationErrors, t }: {
  ...
  editable?: boolean;
  field?: keyof ChargeType;
  required?: boolean;
  hasError: (f: string) => boolean;
  currentItem: Partial<ChargeType>;
  setCurrentItem: (c: Partial<ChargeType>) => void;
...
export default function ChargeTypePage() {
  ...
  const [currentItem, setCurrentItem] = useState<Partial<ChargeType>>({});
  ...
  const handleDelete = async () => {
    ...
    try {
      await chargeTypeAPI.deleteChargeType(deleteTarget.id);
  ...
  const handleAdd = async (item: Partial<ChargeType>) => {
    const itemName = item.name || '';
    try {
      const created = await chargeTypeAPI.createChargeType({ name: item.name || '', status: item.status || 'active' });
  ...
  const handleEdit = async (item: Partial<ChargeType>) => {
    if (!currentItem.id) return;
    const itemName = currentItem.name || '';
    try {
      await chargeTypeAPI.updateChargeType(currentItem.id, item);
      setItems(prev => prev.map(r => r.id === currentItem.id ? { ...r, ...item } as ChargeType : r));
  ...
  const renderFormContent = () => (
    <>
      ...
      <div className="flex items-center gap-4 pb-5 border-b border-gray-100 dark:border-gray-800/50">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 capitalize mb-1">{t('chargeType')}<span className="text-red-500 ml-0.5">*</span></p>
          <input
            type="text"
            ...
            onChange={(e) => { setCurrentItem({ ...currentItem, name: e.target.value }); if (validationErrors.name) { const err = { ...validationErrors }; delete err.name; setValidationErrors(err); } }}
          />
        </div>
  ...
  const validateRequired = (): boolean => {
    ...
    if (!currentItem.name?.trim()) errors.name = language === 'zh' ? '费用类型' : 'Charge Type';
    ...
    <div className="space-y-3">
      ...
      <div className="sticky -top-4 z-10 -mx-6 px-6 pt-4 pb-3 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-white">{t('chargeType')}</h1>
            <p className="text-sm text-gray-500">{t('manageChargeTypes')}</p>
          </div>
            ...
            <tbody className="divide-y divide-gray-100/80 dark:divide-gray-800/50">
              {isLoading ? (
                Array.from({ length: pageSize }).map((_, i) => (
                  <tr key={`skeleton-${i}`}>
                    <td className="px-5 py-3 whitespace-nowrap"><div className="flex items-center gap-3"><div className="skeleton w-8 h-8 rounded-lg flex-shrink-0" /><div className="skeleton h-4 w-28 rounded" /></div></td>
                  ...
                  <motion.tr key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="group hover:bg-blue-50/30 dark:hover:bg-gray-800/30 transition-colors">
                    <td className="px-5 py-3 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg avatar-container text-xs flex-shrink-0">{item.name.charAt(0)}</div>
                        <div className="font-semibold text-[13px] text-gray-900 dark:text-white">{item.name}</div>
                      </div>
                    </td>
                    <td className="px-5 py-3 whitespace-nowrap">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4" onClick={() => { setIsDetailModalOpen(false); setCurrentItem({}); }}>
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: 'spring', duration: 0.3, bounce: 0.15 }} className="modal-content relative bg-white dark:bg-gray-900/95 rounded-2xl border border-gray-100 dark:border-gray-800/80 w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6 border-b border-gray-100 dark:border-gray-800/80 flex items-center justify-between">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">{language === 'zh' ? '费用类型详情' : 'Charge Type Details'}</h3>
              ...
              <div className="p-6 space-y-5">
                <div className="flex items-center gap-4 pb-5 border-b border-gray-100 dark:border-gray-800/50">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-base font-bold text-gray-900 dark:text-white truncate">{currentItem.name}</h4>
                  </div>
                ...
                <div className="rounded-xl bg-gradient-to-br from-gray-50 to-gray-50/50 dark:from-gray-800/30 dark:to-gray-800/10 border border-gray-100 dark:border-gray-800/50 p-5 space-y-4">
                  <div className="flex items-center gap-2 mb-1">
                    <div className="w-6 h-6 rounded-md btn-gradient flex items-center justify-center"><i className="fa-solid fa-tags text-[10px] text-white"></i></div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white capitalize">{language === 'zh' ? '费用类型信息' : 'Charge Type Info'}</h4>
                  </div>
                  <DetailField {...dfProps} label={t('chargeType')} value={currentItem.name} field="name" />
                </div>
              </div>
        ...
        {isAddModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4" onClick={() => setIsAddModalOpen(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: 'spring', duration: 0.3, bounce: 0.15 }} className="modal-content relative bg-white dark:bg-gray-900/95 rounded-2xl border border-gray-100 dark:border-gray-800/80 w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <div className="absolute top-0 left-0 right-0 h-1 btn-gradient" />
              <div className="p-6 border-b border-gray-100 dark:border-gray-800/80 flex items-center justify-between">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('add')} {t('chargeType')}</h3>
        ...
        {isEditModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4" onClick={() => setIsEditModalOpen(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: 'spring', duration: 0.3, bounce: 0.15 }} className="modal-content relative bg-white dark:bg-gray-900/95 rounded-2xl border border-gray-100 dark:border-gray-800/80 w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <div className="absolute top-0 left-0 right-0 h-1 btn-gradient" />
              <div className="p-6 border-b border-gray-100 dark:border-gray-800/80 flex items-center justify-between">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('edit')} {t('chargeType')}</h3>
