...
import { ModuleItem } from '../services/mockData';
...
export default function ModulePage() {
  ...
  const fetchItems = async () => { setIsLoading(true); try { const data = await moduleAPI.getModuleItems(); setItems(data); } catch { toast.error(t('fetchError') || '获取数据失败'); } finally { setIsLoading(false); } };

  const handleDelete = async () => {
    if (!deleteTarget) return;
    const delName = deleteTarget.name;
    try { await moduleAPI.deleteModuleItem(deleteTarget.id); setItems(prev => prev.filter(r => r.id !== deleteTarget.id)); showToast(language === 'zh' ? '删除成功' : 'Deleted Successfully', language === 'zh' ? `${delName} 已被删除` : `${delName} has been deleted`, 'delete'); } catch { toast.error(t('deleteError') || '删除失败'); } finally { setDeleteTarget(null); }
  };
  ...
  const handleAdd = async (item: Partial<ModuleItem>) => {
    const n = item.name || '';
    try { const created = await moduleAPI.createModuleItem({ name: n, status: item.status || 'active' }); setItems(prev => [...prev, created]); setIsAddModalOpen(false); setCurrentItem({}); showToast(language === 'zh' ? '添加成功' : 'Added Successfully', language === 'zh' ? `${n} 已成功添加` : `${n} has been added`, 'add'); } catch { toast.error(t('addError') || '添加失败'); }
  };

  const handleEdit = async (item: Partial<ModuleItem>) => {
    if (!currentItem.id) return;
    const n = currentItem.name || '';
    try { await moduleAPI.updateModuleItem(currentItem.id, item); setItems(prev => prev.map(r => r.id === currentItem.id ? { ...r, ...item } as ModuleItem : r)); setIsEditModalOpen(false); setCurrentItem({}); showToast(language === 'zh' ? '更新成功' : 'Update Successful', language === 'zh' ? `${n} 的信息已成功更新` : `${n} has been updated`, 'update'); } catch { toast.error(t('updateError') || '更新失败'); }
  };
    ...
    <>
      ...
      <div className="flex items-center gap-4 pb-5 border-b border-gray-100 dark:border-gray-800/50">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 capitalize mb-1">{t('module')}<span className="text-red-500 ml-0.5">*</span></p>
  ...
  const validateRequired = (): boolean => { const errors: Record<string, string> = {}; if (!currentItem.name?.trim()) errors.name = language === 'zh' ? '模块' : 'Module'; setValidationErrors(errors); return Object.keys(errors).length === 0; };
    ...
    <div className="space-y-3">
      <div className="sticky -top-4 z-10 -mx-6 px-6 pt-4 pb-3 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div><h1 className="text-xl font-bold text-gray-900 dark:text-white">{t('module')}</h1><p className="text-sm text-gray-500">{t('manageModules')}</p></div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            <thead><tr className="bg-gradient-to-r from-gray-50/80 to-gray-100/50 dark:from-gray-800/40 dark:to-gray-800/20 border-b border-gray-200/60 dark:border-gray-700/50">
              <th scope="col" className="px-5 py-3 text-left text-sm font-bold text-gray-900 dark:text-gray-200 capitalize">{t('module')}</th>
      ...
      <AnimatePresence>{isDetailModalOpen && currentItem.id && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4" onClick={() => { setIsDetailModalOpen(false); setCurrentItem({}); }}>
          <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: 'spring', duration: 0.3, bounce: 0.15 }} className="modal-content relative bg-white dark:bg-gray-900/95 rounded-2xl border border-gray-100 dark:border-gray-800/80 w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 border-b border-gray-100 dark:border-gray-800/80 flex items-center justify-between"><h3 className="text-lg font-bold text-gray-900 dark:text-white">{language === 'zh' ? '模块详情' : 'Module Details'}</h3><button className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors" onClick={() => { setIsDetailModalOpen(false); setCurrentItem({}); }}><i className="fa-solid fa-xmark"></i></button></div>
      ...
      <AnimatePresence>{isAddModalOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4" onClick={() => setIsAddModalOpen(false)}>
          <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: 'spring', duration: 0.3, bounce: 0.15 }} className="modal-content relative bg-white dark:bg-gray-900/95 rounded-2xl border border-gray-100 dark:border-gray-800/80 w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="absolute top-0 left-0 right-0 h-1 btn-gradient" />
            <div className="p-6 border-b border-gray-100 dark:border-gray-800/80 flex items-center justify-between"><h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('add')} {t('module')}</h3><button className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors" onClick={() => { setIsAddModalOpen(false); setCurrentItem({}); }}><i className="fa-solid fa-xmark"></i></button></div>
      ...
      <AnimatePresence>{isEditModalOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4" onClick={() => setIsEditModalOpen(false)}>
          <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }} transition={{ type: 'spring', duration: 0.3, bounce: 0.15 }} className="modal-content relative bg-white dark:bg-gray-900/95 rounded-2xl border border-gray-100 dark:border-gray-800/80 w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="absolute top-0 left-0 right-0 h-1 btn-gradient" />
            <div className="p-6 border-b border-gray-100 dark:border-gray-800/80 flex items-center justify-between"><h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('edit')} {t('module')}</h3><button className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors" onClick={() => { setIsEditModalOpen(false); setCurrentItem({}); }}><i className="fa-solid fa-xmark"></i></button></div>
            <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">{renderFormContent()}</div>
            <div className="p-6 border-t border-gray-100 dark:border-gray-800/80 flex justify-end space-x-3">
